...
func (au *AccountUpdate) SetUsernameID(id int) *AccountUpdate {
	au.mutation.SetUsernameID(id)
	return au
}

// SetUsername sets the "username" edge to the User entity.
func (au *AccountUpdate) SetUsername(u *User) *AccountUpdate {
	return au.SetUsernameID(u.ID)
}
...
func (au *AccountUpdate) check() error {
	...
	if _, ok := au.mutation.UsernameID(); au.mutation.UsernameCleared() && !ok {
		return errors.New(`ent: clearing a required unique edge "Account.username"`)
	}
	return nil
}
...
func (au *AccountUpdate) sqlSave(ctx context.Context) (n int, err error) {
	...
	if au.mutation.UsernameCleared() {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2O,
			Inverse: true,
			Table:   account.UsernameTable,
			Columns: []string{account.UsernameColumn},
			Bidi:    false,
	...
	if nodes := au.mutation.UsernameIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2O,
			Inverse: true,
			Table:   account.UsernameTable,
...
// SetUsernameID sets the "username" edge to the User entity by ID.
func (auo *AccountUpdateOne) SetUsernameID(id int) *AccountUpdateOne {
	auo.mutation.SetUsernameID(id)
	return auo
}

// SetUsername sets the "username" edge to the User entity.
func (auo *AccountUpdateOne) SetUsername(u *User) *AccountUpdateOne {
	return auo.SetUsernameID(u.ID)
}
...
func (auo *AccountUpdateOne) check() error {
	...
	if _, ok := auo.mutation.UsernameID(); auo.mutation.UsernameCleared() && !ok {
		return errors.New(`ent: clearing a required unique edge "Account.username"`)
	}
	return nil
}
...
func (auo *AccountUpdateOne) sqlSave(ctx context.Context) (_node *Account, err error) {
	...
	if nodes := auo.mutation.UsernameIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2O,
			Inverse: true,
			Table:   account.UsernameTable,
